...
- `data/programs_catalog.json` is the canonical **file** catalog; `scripts/sync_programs_catalog.py` validates JSON and upserts to Supabase. Runtime `GET /api/programs` reads DB first, JSON fallback. Dashboard loads the catalog from API / `demo-programs.json` — no embedded JS duplicate.
